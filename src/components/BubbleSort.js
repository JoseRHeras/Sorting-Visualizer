    ...
    for(var i = 0; i < arr.length; i++){
        for(var j = 0; j < arr.length - i - 1; j++){
            ...
            if(arrayCopy[j] > arrayCopy[j + 1]){
                ...
                // coloredArray.push([j, j + 1, arrayCopy[j], arrayCopy[j + 1]]);      //Array saves indexes and their repesctive swapped value
                tempArrayContainer.push(arrayCopy[j], arrayCopy[j + 1]);
            }
